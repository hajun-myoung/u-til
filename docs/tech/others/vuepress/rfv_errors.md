...
### 문제점
문제는, `echo` 명령에 있었어요.  
윈도우의 cli(파워쉘이든, cmd든 모두 해당돼요.)의 기본적인 텍스트인코딩과 웹 표준이 달랐죠.  
- 윈도우: **utf-16**로
- 웹 표준: **utf-8** 이죠.  

인코딩 방식이 달라, 웹팩이 우리가 부탁한 .md 파일을 컴파일 할 때 깨진 텍스트를 뿜을 수 밖에 없어요.  
...
그리고 무엇보다, utf-8 기준으로 이미 깨져있는 텍스트를 읽어봤자 그걸 그대로 보여줄 뿐이랍니다....ㅎㅎ..ㅎ.
:::